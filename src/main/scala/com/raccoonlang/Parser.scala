...
object AstNodeId {
  // Synthetic ids share the SourceId allocator with parses. This assumes single-threaded checking,
...
object Span {
  ...
  /**
   * A span for a term the checker fabricates rather than reads from source. Node identity is carried by `span.nodeId`
   * alone, so a fabricated term gets its fresh `AstNodeId.synthetic()` by getting a span built around one — which keeps
   * the identity discipline of docs/kernel.md#value-identity without a second id field on every node.
   */
  def synthetic(): Span = {
    val id = AstNodeId.synthetic()
    Span(id.start, id.start, id.source)
  }
}
...
object Parser {
  ...
  class FilterParser[T](p: Parser[T], f: T => Boolean) extends Parser[T] {
    override def parse(input: String, startIdx: Int): ParseResult[T] = p.parse(input, startIdx) match {
      ...
      case _                                    => fail(startIdx, startIdx)
    }
  }

  /**
   * Makes `p` fully backtrackable: a cut inside it (`~/`, which throws `ParseError`) becomes an ordinary failure at the
   * starting position. Needed where a prefix that commits internally is only a *candidate* — a parameter list shares
   * its prefix `(x: T)` with a Pi binder, and only the `->` after it decides which was written.
   */
  case class Attempt[T](p: Parser[T]) extends Parser[T] {
    override def parse(input: String, startIdx: Int): ParseResult[T] =
      try p.parse(input, startIdx)
      catch { case e: ParseError => Failure(startIdx, e.curIdx, e.message) }

    override def toString: String = s"($p).attempt"
  }
  ...
  object Rep {
    ...
    def repParser[A](p: Parser[A], min: Int, acc: AccumulatorMaker[A]): Parser[acc.Out] = new Parser[acc.Out] {
      override def parse(input: String, startIdx: Int): ParseResult[acc.Out] = {
        val inst = acc.make()
        var stopped: Failure = null
        var curIdx = startIdx

        while (stopped eq null) {
        ...
        // Falling short of `min` reports the failure that ended the repetition: it already says
        // where `p` stopped, and building a fresh message from this parser's printed tree would be
        // paid on every speculative alternative that turns out not to apply.
        if (inst.length >= min) Success(inst.res, startIdx, curIdx)
        else Failure(startIdx, stopped.curIdx, stopped.message)
      }
