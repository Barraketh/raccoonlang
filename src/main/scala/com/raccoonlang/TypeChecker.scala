...
object TypeChecker {
  ...
  private def checkApplyValue(fn: Value, args: Vector[Value], normalizerMap: Normalizers.NormalizerMap)(implicit
      eqStore: EqStore
  ): Value =
    fn.tpe.caseOf {
      case pi: VPi =>
  ...
  private def checkPi(pi: CA.Term.Pi, env: Env)(implicit
      meta: EqStore
  ): CheckedPi = {
  ...
  def checkRef(ref: CA.Term.Ref, env: Env)(implicit
      meta: EqStore
  ): Value = {
  ...
  private def checkSelect(baseValue: Value, field: String, span: Span, env: Env)(implicit
      eqStore: EqStore
  ): Value = {
  ...
  private def checkLam(l: CA.Term.Lam, env: Env)(implicit
      eqStore: EqStore
  ): VLam = {
    ...
    val (checkedBody, bodyTerm) = l.body match {
      ...
      case _ =>
        val bodyV = check(l.body, recurEnv)(eqStore)
    ...
    val checkedLam =
      EA.Term.Lam(quotedPi, Vector.empty, bodyTerm, l.span, l.name, l.isStable)
    Interpreter.evalLam(checkedLam, vpi, env)
  }

  def getType(term: CA.TypeTerm, env: Env)(implicit ctx: EqStore): Value = {
