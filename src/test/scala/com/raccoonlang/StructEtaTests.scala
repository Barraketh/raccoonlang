...
class StructEtaTests extends munit.FunSuite {
  ...
  test("binder eta expands to constructor of projections") {
    val src = eq + nat + pair +
      "def eta (p: Pair(Nat, Nat)): Eq(Pair(Nat, Nat), p, Pair.mk(Nat, Nat, p.fst, p.snd)) := Eq.refl(Pair(Nat, Nat), p)\n"
  ...
  test("plain one-constructor inductive eta uses explicit match accessors") {
    val src = eq + nat +
      ...
      "def eta (p: Plain(Nat, Nat)): Eq(Plain(Nat, Nat), p, Plain.mk(Nat, Nat, getLeft(p), getRight(p))) := Eq.refl(Plain(Nat, Nat), p)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("dependent fields preserve the dependent eta view") {
    val src = eq +
      ...
      "def eta (s: Sig(A, B)): Eq(Sig(A, B), s, Sig.mk(A, B, s.fst, s.snd)) := Eq.refl(Sig(A, B), s)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }

  test("opaque constant eta") {
    val src = eq + nat + pair +
      "opaque def c : Pair(Nat, Nat) := Pair.mk(Nat, Nat, Nat.zero, Nat.zero)\n" +
      "def eta : Eq(Pair(Nat, Nat), c, Pair.mk(Nat, Nat, c.fst, c.snd)) := Eq.refl(Pair(Nat, Nat), c)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("axiom eta") {
    val src = eq + nat + pair +
      ...
      "def eta : Eq(Pair(Nat, Nat), a, Pair.mk(Nat, Nat, a.fst, a.snd)) := Eq.refl(Pair(Nat, Nat), a)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("unit-like eta makes all values equal") {
    val src = eq +
      "struct One : Type\n | mk : One\n\n" +
      "def unique (a: One)(b: One): Eq(One, a, b) := Eq.refl(One, a)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("blocked struct-valued match eta") {
    val src = eq + nat + pair +
      ...
      " | Nat.succ k => Pair.mk(Nat, Nat, k, n)\n}\n" +
      "def eta (n: Nat): Eq(Pair(Nat, Nat), pick(n), Pair.mk(Nat, Nat, pick(n).fst, pick(n).snd)) := Eq.refl(Pair(Nat, Nat), pick(n))\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("matching a struct binder fires with virtual projected fields") {
    val src = eq + nat + pair +
      ...
      " | Pair.mk fst snd => Pair.mk(Nat, Nat, snd, fst)\n}\n" +
      "def eta (p: Pair(Nat, Nat)): Eq(Pair(Nat, Nat), swap(p), Pair.mk(Nat, Nat, p.snd, p.fst)) := Eq.refl(Pair(Nat, Nat), swap(p))\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("nested structure fields expand recursively") {
    val src = eq + nat + pair +
      ...
      "def eta (b: Box(Pair(Nat, Nat))): Eq(Pair(Nat, Nat), b.inner, Pair.mk(Nat, Nat, b.inner.fst, b.inner.snd)) := Eq.refl(Pair(Nat, Nat), b.inner)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("concrete constructor evaluation is unchanged") {
    val src = nat + pair +
      ...
      " | Nat.zero => Pair.mk(Nat, Nat, Nat.zero, Nat.succ(Nat.zero))\n" +
      " | Nat.succ k => Pair.mk(Nat, Nat, k, n)\n}\n"
  ...
  test("recursive structures are not eta-eligible") {
    val src = eq + "struct Wrap : Type\n | mk (w: Wrap) : Wrap\n\n" +
      "def eta (x: Wrap): Eq(Wrap, x, Wrap.mk(x.w)) := Eq.refl(Wrap, x)\n"
  ...
  test("user selectors equal canonical kernel projections") {
    val src = eq + nat + pair +
      ...
      "def selector (p: Pair(Nat, Nat)): Eq(Nat, Pair.fst(Nat, Nat, p), p.fst) := Eq.refl(Nat, p.fst)\n" +
      "def eta : Eq(Pair(Nat, Nat), p, Pair.mk(Nat, Nat, p.fst, p.snd)) := Eq.refl(Pair(Nat, Nat), p)\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("a variable refined to a structure gets eta in its branch") {
    val src = eq + nat + pair +
      ...
      " | Tag.isPair => { let same : Eq(Pair(Nat, Nat), x, Pair.mk(Nat, Nat, x.fst, x.snd)) := Eq.refl(Pair(Nat, Nat), x) x.fst }\n}\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }
  ...
  test("termination measure decreases through a projected field") {
    val src = nat + pair +
      ...
      " | Nat.zero => Nat.zero\n" +
      " | Nat.succ k => countdown(Pair.mk(Nat, Nat, k, p.snd))\n}\n"
    TypeChecker.checkProgram(TestSupport.core(src))
  }

  test("distinct flat struct neutrals remain unequal") {
    val src = eq + nat + pair +
      "def bad (a: Pair(Nat, Nat))(b: Pair(Nat, Nat)): Eq(Pair(Nat, Nat), a, b) := Eq.refl(Pair(Nat, Nat), a)\n"
  ...
  test("distinct nested struct neutrals remain unequal and terminate") {
    val src = eq + nat + pair +
      "def bad (a: Pair(Pair(Nat, Nat), Nat))(b: Pair(Pair(Nat, Nat), Nat)): Eq(Pair(Pair(Nat, Nat), Nat), a, b) := Eq.refl(Pair(Pair(Nat, Nat), Nat), a)\n"
    intercept[TypeMismatch](TypeChecker.checkProgram(TestSupport.core(src)))
  }
}
