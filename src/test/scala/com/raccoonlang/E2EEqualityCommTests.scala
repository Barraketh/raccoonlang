  ...
  test("prove add commutativity (a + b = b + a)") {
    val p =
      """
        ...
        |  match p as p0 returning Eq A x z with
        |  | Eq.refl A0 w => q
        |
        |def symm (A: Type)(x: A)(y: A)(p: Eq A x y): Eq A y x :=
        |  match p as p0 returning Eq A y x with
        ...
        |  | Nat.succ x => trans Nat (add (Nat.succ a) (Nat.succ x)) (add (Nat.succ (Nat.succ a)) x) (Nat.succ (add a (Nat.succ x))) (Eq.refl Nat (add (Nat.succ (Nat.succ a)) x)) (trans Nat (add (Nat.succ (Nat.succ a)) x) (Nat.succ (add (Nat.succ a) x)) (Nat.succ (add a (Nat.succ x))) (addSuccLeft (Nat.succ a) x) (Eq.refl Nat (Nat.succ (add (Nat.succ a) x))))
        |
        |def addZeroLeft (a: Nat): Eq Nat (add Nat.zero a) a :=
        |  match a as a0 returning Eq Nat (add Nat.zero a0) a0 with
