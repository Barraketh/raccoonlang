...
/** Pins the declaration-time proof representation policy (docs/kernel.md#proofs-and-elimination). */
class ProofCollapseTests extends munit.FunSuite with TestSupport {
  ...
  test("unforced implicit proof params are rejected, never auto-discharged") {
    ...
    expectTypeError[NonForcedImplicitParam](
      """
  ...
  test("a rigid-neutral match wakes when its scrutinee sort collapses to Prop") {
    ...
    // Indexed, so structure eta does not apply and the match genuinely sticks on a neutral.
    val polyUnitAtU = Interpreter.evalApply(env("PolyUnit"), Vector(u, env("Peano.zero")))
    ...
    // Construct the blocked application directly so the scrutinee is a genuine blocked neutral.
  ...
  test("proof-valued lambdas become canonical eta-lambdas without rewriting their checked residual") {
    ...
    val residual = checked.residual match {
      case CoreAst.Term.Body(Vector(), res, _) => res
      case other                               => other
    }
    residual match {
      case CoreAst.Term.Lam(
            outerType,
            CoreAst.Term.Lam(innerType, _: CoreAst.Term.App, _, _, _, _),
            _,
            _,
            _,
            _
          ) =>
        assertEquals(outerType.knownPropValued, Some(true))
        assertEquals(innerType.knownPropValued, Some(true))
      case other => fail(s"Expected the original constructor application in the checked residual, got $other")
    }
    Interpreter.evalTerm(residual, Prelude.default.checkedEnv) match {
      case Value.VLam(_, _, Value.LamBody.ProofEta) =>
      case other => fail(s"Expected the re-evaluated residual to retain proof-lambda canonicalization, got $other")
    }
  }
  ...
  test("shallow recovery keeps dependent Pi proof fields applicable") {
    typecheckDecls(
      """
        |inductive DependentProof : Prop
        | | mk (h: (n: Nat) -> True) (same: Eq(True, h(Nat.zero), h(Nat.zero))) : DependentProof
        |
  ...
  test("erased proofs re-evaluate from their residual to their canonical proof form") {
    // Residuals are pure syntax, so a proof reaches a later evaluation through the syntax that
    // produced it rather than as a carried value.
    val res = runProgram(
      """
        |axiom P : Prop
        |axiom axP : P
    ...
    res match {
      case p: Value.VProof => assert(Value.isPropositionType(p.tpe))
      case other           => fail(s"Expected a proof of P, got $other")
    }
  }

  test("annotated let proof values stay collapsed through evalBody") {
    // A let's value is bound as evaluated — nothing retypes it at the annotation — so the value
    // must already be collapsed when it is produced. `putLocal`'s collapse-invariant assertion
    // would reject an uncollapsed prop-typed binding.
    val res = runProgram(
      """
        |axiom P : Prop
        |axiom axP : P
